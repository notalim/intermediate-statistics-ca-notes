---
title: "Intermediate Statistics CA RStudio Notes"
author: "Dan Liu & Bryan Chan"
date: "`r Sys.Date()`"
output: pdf_document
---
...
> These notes are meant to help you succeed in this course. These will include necessary tips for *R* formulas you will need throughout this course. Feel free to give feedback on these via our emails: [dliu4\@stevens.edu](mailto:dliu4@stevens.edu){.email},
[bchan4\@stevens.edu](mailto:bchan4@stevens.edu){.email},
do not contact these graduates ([akassymo\@stevens.edu](mailto:akassymo@stevens.edu){.email},'
[aleather\@stevens.edu](mailto:aleather@stevens.edu){.email}).

> **These notes are not meant to replace the slides.** For more info, please refer to the slides.

## Installation of R Studio (and R Markdown)
To install RStudio, head to <https://posit.co/download/rstudio-desktop/> and follow the instructions on the page. You can also install it with your package manager of choice.

Alternatively, you can sign up for a free account on Posit Cloud (<https://posit.cloud/>) and get up to 25 hours of compute a month, which should be enough for this course.

**We highly suggest installing *R Markdown*!** It will make your life so much easier. Once you've downloaded RStudio, follow this great guide to find out how to install R Markdown:
<https://alexd106.github.io/intro2R/install_rmarkdown.html/>

You can use *this* very document to learn about how notation gets rendered. Check out the .Rmd file of this pdf.

In general, R Markdown is basically equivalent to regular Markdown (<https://www.markdownguide.org/>). However, the key differences are that you can add inline and blocks of LaTeX code for symbolic notation, and most importantly, write code blocks like the one seen under section Lecture 1.

The output of these code blocks will be present in the rendered document unless you have set the right options inside the curly braces. Check out the Cookbook page 11.7 "Hide code, text output, messages, or plots"

```{r, echo=FALSE, results=FALSE}
# you found me
print("you found me")
# notice how the code and the output of this code block is missing from the pdf
```

# Helpful Resources

The R Markdown Cookbook (<https://bookdown.org/yihui/rmarkdown-cookbook/>) is a great resource I used when taking this course. I didn't end up using too many of the formatting options, but feel free to make your work look as nice as you want.

> **Tip**: If you want to keep all your code and data in the same place, remember to click "Session > Set Working Directory > To Source File Location". This allows you to access files in the folder that your code file is in. So, loading data will only require a short relative path and not the full path.
...
```{r}
# this is how a code block should look, just remove the first # on each line.
# ```{r}
# # *code here*
# ```
...
In *R Markdown* you can easily format math formulas using LaTeX! To do that, you just have to use \$ signs from both sides of an equation. There are a couple more cool things you could do inside the math equation:

1.  Use an underscore \_{\<index\>} to add an index/subscript to your equation: `$H_{0}$` would give you $H_{0}$.
...
To learn more about how to format math in LaTeX, please take a look under the Mathematics section of the sidebar on [Overleaf Learn](https://www.overleaf.com/learn)
...
### Student $\mathcal{T}$-distribution
...
1.  We first need a binary sample `x` where "1" indicates a success and "0" indicates a failue (refer to Population proportion intervals in Lecture 3 on how to convert a dataset to binary)
...
3.  We then calculate the p-value based on the side of the test.
    ...
    b.  for left-tailed tests where $H_a: \mu_1 < \mu_2$, we use `p = pnorm(z, 0, 1)`
...
2.  We can now calculate the p-value using the following R function: `t.test(x, y, alternative=c("side"), paired=FALSE, var.equal=TRUE, conf.level=1-alpha)`
...
2.  We can now calculate the p-value using the following R function: `t.test(x, y, alternative=c("side"), paired=FALSE, var.equal=FALSE, conf.level=1-alpha)`
...
**I do not recommend doing it this way. The R function will generally lead to less mistakes.** 1. Identify your two sample variances `var1` and `var2`, and the size of each sample `n1` and `n2`. 2. Calculate $f$ `f = var1 / var2` 3. Calculate p-value:
...
### Comparing population proportions through confidence intervals
...
We use the function `chisq.test(table, correct=FALSE)` for a dataset `table` and we do not use Yate's correction. Yate's correction is used when one of the entries in the dataset is less than 5. You shouldn't need to use this correction in this course.
...
There is a one-way ANOVA test and a regular ANOVA. The first one means that we're testing only one independent variable. In general, one-way ANOVA test is just an $F$-test, where $df_1 = k - 1$ and $df_2 = n - k$. Regular full ANOVA uses the same method on every group.
...
-   $SSB$ (Sum of Squares Between) is a sum of squares of the differences between the sample mean and respective group means for every element in the sample.
...
$p = P_{H_{0}}(F > f) = 1 - \text{pf}(f, k - 1, n - k)$
...
Using this formula, we can estimate contrast using sample means, assuming groups are independent:

$\psi = a_1\overline{X}_1 + ... + a_k\overline{X}_k$

Similarly, pooled sample variance $S^2_p$ can be estimated through $MSE$. To test our estimations, we can use a $\mathcal{T}$-test and $F$-test.

> I'm not going to go into an example of contrast inference, because it's a regular $\mathcal{T}$-test. For a good example, refer to Slides 7.
...
1.  `tapply(response, factor, length)` to get the sizes of each category in the dataset.
2.  Create a vector of patterns we want to test: `pattern = cbind(c(1, 1, -1, -1, -1, 1), c(0, 0, -1, 2, -1, 0))`. In this case, let's assume for the first pattern, groups 2, 3 and 4 are different from groups 0, 1 and 5. For the second pattern, we separate groups 0, 1, 5 and groups 2, 4 from the biggest outlier group 3.
...
3.  To identify the groups, look at the bottom of the test (example from the slides):

![lsd-test-example](img/lsd-test-example.png)
...
To make a linear regression model using R, we first must identify our dataset `data`, independent variable `x`, and independent variable `y`. We can then write the following code:
